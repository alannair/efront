...
function onDeleteForumMessage(el, response) {
	if (response.evalJSON(true).deleted_topic == true) {
		window.location = response.evalJSON(true).redirect_url;
	} else {
...
function addAdditionalChoice() {
    ...
    if (counter > 1) {                                                  //If the counter is less than 2 (where 2 is the default input fields), it means that the selected question type is not one that may have multiple inputs (i.e. it may be raw_text)
        ...
        var img = new Element('img', {alt: removechoice, title: removechoice, src: 'themes/default/images/others/transparent.gif'}).addClassName('sprite16').addClassName('sprite16-error_delete').setStyle({whiteSpace: 'nowrap'}).addClassName('ajaxHandle');						//Create an image element, that will hold the "delete" icon
